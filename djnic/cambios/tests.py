...
class CambioDominioTestCase(TestCase):
    ...
    def test_change_new_domain(self):
        ...
        self.assertEqual(CambiosDominio.objects.all().count(), 0)

        dominio2 = Dominio.add_from_whois('fernet.com.ar', mock_from_txt_file='djnic/whosamples/sample_fernet_updated.txt')
        self.assertEqual(CambiosDominio.objects.all().count(), 1)
        
        main_change = CambiosDominio.objects.all()[0]
        self.assertEqual(CampoCambio.objects.filter(cambio=main_change, campo='dominio_registered', anterior='2020-05-07 13:44:04', nuevo='2021-05-07 10:44:04').count(), 1)
        self.assertEqual(CampoCambio.objects.filter(cambio=main_change, campo='dominio_changed', anterior='2020-05-07 17:34:40', nuevo='2021-05-07 14:34:40').count(), 1)
        self.assertEqual(CampoCambio.objects.filter(cambio=main_change, campo='dominio_expire', anterior='2021-05-07 03:00:00', nuevo='2022-05-07 00:00:00').count(), 1)
        ...
        self.assertEqual(CampoCambio.objects.filter(cambio=main_change, campo='registrant_created', anterior='2013-08-19 03:00:00', nuevo='2020-08-19 00:00:00').count(), 1)
        self.assertEqual(CampoCambio.objects.filter(cambio=main_change, campo='registrant_changed', anterior='2020-10-11 20:37:54', nuevo='2020-10-11 17:37:54').count(), 1)
